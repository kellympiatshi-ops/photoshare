...
import { Camera, Plus, FolderOpen, ArrowLeft, Lock } from 'lucide-react';
...
export function ClientDashboard({ onBack }: ClientDashboardProps) {
        ...
        for (const id of collectionIds) {
          try {
            ...
            const response = await fetch(
              `https://${projectId}.supabase.co/functions/v1/make-server-66ce0d40/collections/${id}`,
              {
                headers: {
  ...
  const handleAccessCollection = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-66ce0d40/collections/access`,
        {
          method: 'POST',
          headers: {
