...
import { Camera, Plus, FolderOpen, ArrowLeft, Copy, Trash2 } from 'lucide-react';
...
export function PhotographerDashboard({ onBack }: PhotographerDashboardProps) {
  ...
  const loadCollections = async () => {
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-66ce0d40/collections?sessionId=${sessionId}`,
        {
          headers: {
      ...
      if (data.collections) {
        ...
        localStorage.setItem('cached_collections', JSON.stringify(data.collections));
  ...
  const getOrCreateSessionId = () => {
    let sessionId = localStorage.getItem('photographer_session_id');
    if (!sessionId) {
      sessionId = crypto.randomUUID();
      localStorage.setItem('photographer_session_id', sessionId);
    }
    return sessionId;
  };
  ...
  const handleCreateCollection = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const sessionId = getOrCreateSessionId();
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-66ce0d40/collections`,
        {
          method: 'POST',
          headers: {
  ...
  const handleDeleteCollection = async (collectionId: string) => {
    ...
    try {
      const sessionId = getOrCreateSessionId();
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-66ce0d40/collections/${collectionId}?sessionId=${sessionId}`,
        {
          method: 'DELETE',
          headers: {
            Authorization: `Bearer ${publicAnonKey}`,
